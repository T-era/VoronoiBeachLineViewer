(function() {
	Voronoi.BeachLine = function(arg, logger) {
		...
		var seed = uniqueList(arg, function(p1, p2) {
			return d_same(p1.x, p2.x, 3)
				&& d_same(p1.y, p2.y, 3);
		}, function(p) { return new Voronoi.MPoint(p); });
	...
	Voronoi.BeachLine.prototype.stepNextEvent = function() {
		var event = null;
		var done = false;
		...
		if (nextCircle) { // サークルイベントが起きないなら、サイトイベントを起こす。
			event = Voronoi.BeachLine.toCircleEvent(nextCircle, this.voronoiPoints);
		} else {
			var nextPoint = this.getNextSight();
			if (nextPoint) {
				event = Voronoi.BeachLine.toSightEvent(nextPoint, this.voronoiPoints);
				this.sightPointIndex ++;
			} else {
				event = {
					eventBorder: Voronoi.WorldSize.height * 2,
	...
	Voronoi.BeachLine.prototype.getNextCircleEvent = function() {
		var nextSight = this.getNextSight();
		// Seek circle event
	...
	Voronoi.BeachLine.prototype.toList = function(fSort) {
		var list = [];
		if (this.topNode) {
